...
function loadAllQuestions() {
    ...
    const promises = quizConfig.modules.map(module => {
        // Ao carregar arquivos localmente (via file://), é necessário construir o caminho
        // absoluto para evitar erros de CORS. Se a página estiver sendo servida a partir
        // de um servidor (http/https), o fetch funcionará com caminhos relativos.
        let url = `${module.file}.json`;
        if (typeof window !== 'undefined' && window.location && window.location.protocol === 'file:') {
            // Obtém o diretório atual do arquivo index.html (inclui a barra final)
